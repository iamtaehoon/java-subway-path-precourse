...
경로를 조회할지 종료할지 결정한다.
  - 다른 입력이 들어오면 예외를 반환한다.

경로 기준을 선택한다.
  - 최단 거리, 최소 시간, 돌아가기 이외에 다른 입력이 들어오면 예외를 반환한다. 
  - 경로 기준이 최단 거리라면 최단거리 다익스트라를 만든다.
  - 경로 기준이 최소 시간이라면 최소시간 다익스트라를 만든다.

출발역을 입력받는다.
도착역을 입력받는다.
  - 해당 역이 존재하지 않으면 예외를 반환한다.
  - 도착역이 출발역과 동일하면 예외를 반환한다.
...
거리
시간
  - 시간의 toString은 @@분, 60분 이상이라면 @시간 @분 형태로 만들어준다.
